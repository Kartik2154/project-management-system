...
const StudentManagement = () => {
  ...
  const handleStatusChange = async (id, status) => {
    // Map friendly status to exact enum string expected by backend
    const statusMap = {
      agreed: "approved",
      rejected: "rejected",
    };
    ...
    if (!mappedStatus) {
      console.error("Unhandled status value:", status);
      return;
    }

    try {
      await requestAPI.update(id, { status: mappedStatus });
      fetchRequests();
    } catch (error) {
      if (error.response) {
      ...
      <div className="min-h-screen bg-gray-900 text-white flex items-center justify-center">
        <div className="flex flex-col items-center p-8 bg-gray-800 rounded-xl shadow-2xl">
          ...
          <p className="text-sm text-gray-500 mt-1">
            Fetching divisions and student records.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-6 md:p-10 relative z-0">
      <div className="max-w-7xl mx-auto relative z-0">
        {/* Modals render above all content for accessibility/z-index */}
        {showRequestsModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-gray-900 p-6 rounded-xl border border-cyan-600/50 w-full max-w-2xl shadow-2xl overflow-y-auto max-h-[80vh]">
                        ...
                        {filteredRequests.length === 0 ? (
                          ...
                          <div className="space-y-4">
                            {filteredRequests.map((req) => (
                              <div
                                key={req._id}
                                className="bg-gray-800/60 p-4 rounded-lg border border-gray-700 text-white"
                              >
                                <p>
        ...
        {showEditModal && (
          <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-gray-900 p-8 rounded-xl border border-cyan-600/50 w-full max-w-lg shadow-2xl">
              <h2 className="text-3xl font-bold text-white mb-6 border-b border-gray-700 pb-3">
                <Edit size={24} className="inline mr-2 text-cyan-400" /> Edit
                Student Details
              </h2>
        ...
        <div className="flex items-center justify-between mb-10 border-b border-cyan-500/30 pb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-gray-400 mt-1 font-light">
                Efficiently manage all student profiles, registrations, and
                divisions.
              </p>
            </div>
          </div>
          <button
            ...
            className="bg-cyan-600/80 hover:bg-cyan-700 px-4 py-2 rounded-lg text-white font-semibold shadow-md hover:shadow-lg transition-all flex items-center gap-2"
            title="View Student Requests"
          >
            <Users size={20} />
            View Requests
          </button>
        ...
        <div className="bg-gray-800/80 backdrop-blur-md p-6 rounded-xl border border-gray-700 shadow-xl mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            ...
            <div className="md:col-span-2 relative">
              ...
              <Search
                size={20}
                className="absolute left-3 top-1/2 mt-2 transform -translate-y-1/2 text-cyan-500"
              />
              <input
                type="text"
        ...
        <div className="bg-gray-800/80 backdrop-blur-md rounded-xl border border-gray-700 shadow-xl overflow-hidden">
          <h3 className="text-xl font-semibold p-6 border-b border-gray-700 text-gray-200">
            ...
            {loading && (
              <Loader2
                size={20}
                className="inline ml-3 animate-spin text-cyan-400"
              />
            )}
          </h3>
            ...
            <table className="w-full table-auto">
              <thead className="bg-gray-700/50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-cyan-400 hidden sm:table-cell">
                    Email
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-cyan-400 hidden md:table-cell">
                    Phone
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-cyan-400">
                    Status
              ...
              <tbody className="divide-y divide-gray-700">
                {localFilteredStudents.map((student) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4 font-semibold text-sm text-gray-100">
                      {student.name}
                    </td>
                    ...
                    <td className="px-6 py-4 text-sm">
                      <span
                        className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold shadow-inner ${
          ...
          {!loading && localFilteredStudents.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-gray-500 mt-1">
                Try adjusting your search term or clearing the division filter.
              </p>
            </div>
          )}
          ...
          {loading && students.length > 0 && (
            <div className="text-center py-8">
              <Loader2
                size={32}
                className="mx-auto animate-spin text-cyan-400 mb-2"
              />
